...
const getStartColumn = () => 500 - limits.minColumn + 1;
...
const limits = {
  ...
  maxRow: Math.max(...definitions.flatMap((x) => x).map((x) => x.row)) + 2,
  minRow: Math.min(...definitions.flatMap((x) => x).map((x) => x.row)),
};

const extendedBase = limits.maxRow;

limits.maxColumn += extendedBase;
limits.minColumn -= extendedBase;
...
map[limits.maxRow] = map[limits.maxRow].map(() => "#");

const fallSand = (row = 0, column = getStartColumn()) => {
  ...
  if (map[row + 1][column] === ".") {
    ...
    if (row === 0 && column == getStartColumn()) {
      console.log("Full");
      return false;
    }
    map[row][column] = "o";
...
console.log(map.flatMap((x) => x).filter((x) => x === "o").length + 1);
