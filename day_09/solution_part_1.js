...
const updateTail = () => {
  const columnDiff = headCell.column - tailCell.column;
  const rowDiff = headCell.row - tailCell.row;
  ...
  walkedCells.add(tailCell.column + "_" + tailCell.row);
};
...
const renderT = () => {
  map[mapLimits.maxCol - tailCell.column][-mapLimits.minRow + tailCell.row] =
    "T";
};
