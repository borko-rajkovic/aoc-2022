...
const buildTree = (commands) => {
  ...
  for (const command of commands) {
    switch (command.command) {
      ...
      case "ls":
        // console.log(
        //   `Content of the dir ${currentDir.path} is: ${command.results}`
        // );
...
const needToDeleteSize = minimumUnusedSpace - (totalDiskSpaceAvailable - treeRoot.size);

console.log(Object.values(sizes).sort((a,b) => a - b).filter(x => x >= needToDeleteSize)[0]);
